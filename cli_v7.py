...
def refresh_popup_print_table(command_properties, command_dict):
    ...
    if 'state_of_table' not in  g_screen_tree_obj:
        g_screen_tree_obj['state_of_table'] = { 'start_x': 0, 'start_y': 0 , 'word': 99999, 'line': -2, 'total_word_in_line': -1, 'total_line': -1 ,'hide_answer_mode': False, 'hide_line_answer': False, 'secret_in_line': False } # line is -2 because when its -1 it will show all x in the generate_cell_box_text function
    g_screen_tree_obj['state_of_table']['secret_in_line'] = False
    ...
    if command_dict['command'] in [ 'n', 'w' ]:
