...
def colored_print(text,context=True,color="WHITE") -> list:
    lines = text.split('\n')
    colored_line = ''
    for line in lines:
        colored_line = color_line(line, context, color)
...
def popup_display(id):
    ...
    for _ in range(0, table_properties['height']-1):
        ...
        print(' '*max_len_of_line)
    print('*'*max_len_of_line)'''
    screen_coordinator('popup',popup_data ,{ 'who_is_on_screen' : who_is_on_screen })
...
def initialize_tree_params(g_screen_obj):
    ...
    height = os.get_terminal_size().lines - 25
...
def initialize_popup_params(g_screen_obj):
    ...
    width  = os.get_terminal_size().columns
    table_properties={'height': height - int( height * .25 ),'width': width - int(width * .5) , 'default_color': 'WHITE'}
    g_screen_obj[mode]['table_properties']=table_properties
    g_screen_obj[mode]['command_options']=command_options    
...
def screen_coordinator(mode, data_table=None ,context_properties=None): #except for mode all other params should be used only for initialization
    ...
    if mode == 'popup':
        ...
        while command_properties['option'] != 'q':
            ...
            command_attribute = None
            command_dict = { 'command': command, 'command_attribute' : command_attribute}
            popup_screen_action_on_command(command_dict,command_properties,command_options)

def popup_screen_action_on_command(command_dict, command_properties,command_options):
...
def refresh_popup_print_table(command_properties):
    ...
    print_table, new_state_of_table = build_popup_array_table(state_of_table, data_table,table_properties,command_properties)
...
def refresh_tree_print_table(mode, command_properties):
    ...
    state_of_table = g_screen_tree_obj['state_of_table']
    print_table, new_state_of_table = build_tree_array_table(state_of_table, data_table,table_properties, command_properties, context_properties)
...
def build_popup_array_table(state_of_table, table,table_properties,command_properties):
    ...
    start_y = init_y 
    print_arr = []
    print_arr += [ '-' * width for i in range(2) ]
    for row in range(1,height-1):
        print_row = ['|','|']
        start_x = init_x
        for col in range(1,width):
            if start_y >=0 and start_y < len(table) and start_x >=0 and start_x < len( table[start_y] ):
                print_row.insert(col, table[start_y][start_x] )
            else:
                print_row.insert(col, ' ' )
            start_x += 1
        print_arr.insert(row,''.join(print_row))
        start_y += 1
    return print_arr, { 'start_x': init_x, 'start_y': init_y } 
    
def build_tree_array_table(state_of_table, table, table_properties, command_properties, context_properties):
    height=table_properties['height']
    width=table_properties['width']
