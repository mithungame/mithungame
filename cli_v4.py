...
def initialize_tree_params(g_screen_obj):
    ...
    command_options = { 'i j k l': "scroll", 'q' : "q for quit", "p": "popup" , "r": "rndm", "h" : "highlight", "m": "minimize"}
    ...
    #width = 30
    if mode == "tree" : 
        command_options = { **command_options,  **{ 'n': 'move to next file', "c": "calendar", "e": "show ephemeral tree" } }
...
def screen_coordinator(mode, data_table=None ,context_properties=None): #except for mode all other params should be used only for initialization
    ...
    if mode in [ 'calendar','tree', 'ephemeral_tree' ]:
        ...
        while command_properties['option'] != 'n':
            ...
            command_properties['option']=command
            command_attribute = None
            if command in ['c','p','h']: # custom , then revert to normal input 
...
def refresh_tree_print_table(mode, command_properties):
    ...
    if command_properties['option']=='m':
