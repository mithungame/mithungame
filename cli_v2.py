...
class color_word:
    ...
    def __str__(self):
        return self.colored_obj['pre_control'] + self.word + self.colored_obj['post_control']
...
def colored_print(text,context=None,color="WHITE"):
    if context is None:
        if args.vannangal == "true":
            ...
            context = False
    text = str(colored_text(text,color,context))
...
def popup_display(id):
    ...
    for _ in range(0, table_properties['height']-1):
        ...
        print(' '*max_len_of_line)
    print('*'*max_len_of_line)'''
    screen_coordinator('popup',popup_data ,{ 'who_is_on_screen' : who_is_on_screen , 'base_data': popup_obj})
...
def display_table(clear_screen = True, cursor_pos = None):
    ...
    for line in table_to_be_printed:
        colored_print(line,True,color)
        no_of_lines_printed+=1
    for k,v in command_options.items():
        print(k+": "+v,end=' ')
    no_of_lines_printed+=1
    for _ in range(os.get_terminal_size().lines - no_of_lines_printed - 5): print(' '*os.get_terminal_size().columns)
    command = ''
...
def initialize_tree_params(g_screen_obj):
    ...
    command_options = { 'i j k l': "scroll", 'q' : "q for quit", "p": "popup" }
...
def initialize_popup_params(g_screen_obj):
    ...
    #table_properties={'height': 40,'width': 15 , 'default_color': 'WHITE'}
    g_screen_obj[mode]['table_properties']=table_properties
    g_screen_obj[mode]['command_options']=command_options    
...
def build_popup_array_table(state_of_table, table,table_properties,command_properties,cell_properties):
    ...
    cell_block = build_cell_block({ **cell_properties, **{'text':'\n'.join(table), 'width': width , 'height': height, 'w_shift': init_x, 'h_shift': init_y }} )
...
def fill_surround_block(mode,table,cell_properties):
    ...
    for _ in range(units):
        for i in range(cp['start_x'], cp['end_x']+1): 
            make_table_entry( table, cp['start_y'], i, h_char )
            make_table_entry( table, cp['end_y'], i, h_char )
        cp['start_y'] += 1
        cp['end_y'] -= 1
        for i in range(cp['start_y'], cp['end_y']+1): 
            make_table_entry( table,i,cp['start_x'], v_char )
            make_table_entry( table,i, cp['end_x'], v_char )
        cp['start_x'] += 1 
        cp['end_x'] -= 1
    '''for i in table:
        print(''.join(i))
...
def generate_cell_box_text(text, cell_properties):
    return text
    
def generate_cell_tree_text(name, cell_properties):
...
def generate_cell_text(cell_properties):
    ...
    if context in ['tree', 'ephemeral_tree']:
        text = cell_properties['obj']['name']
        text =  generate_cell_tree_text(text,cell_properties)
    elif context == 'popup':
        text = cell_properties['text']
        text =  generate_cell_box_text(text,cell_properties)
    else:
        text = cell_properties['text']
        '''no default impl'''
    return format_cell_text(text,cell_properties['end_x']-cell_properties['start_x'],True)
...
def format_cell_text(text,width=100,wrap=True):
    ...
    while curr_line is not None: #dont do empty some line is empty and it stops at that example hello\n\nhi will stop at hello
        #print(curr_line)
        words = list(re.split('(\W)', curr_line))
        words = [i for i in words if i != '' ]
        #print(words)
        carry_over = ''
        add_new_line=True
        for n,each_word in enumerate(words):
            #print('###'+each_word+'###')
            if len(each_word) + running_width <= width:  
                ...
                if len(each_word) > width : # keep looping this till it becomes empty 
                    ...
                    while each_big_word is not None:
                        each_big_curr_word = each_big_word[0: width - running_width]
                        print('big word is ',each_big_curr_word,width,running_width)
                        formatted_text += each_big_curr_word + '\n'
                        if each_big_word[width - running_width : ] : 
                            each_big_word = each_big_word[width - running_width : ]
                            running_width = 0
                        else:
                            each_big_word = None
                            ...
                            '''print('####')
                            if len(each_big_curr_word) == width - running_width:
                                #print('perfect fit')
                                pass #words fit perfectly nothing to do
                            else:
                                add_new_line = False
                                if n == len(words) - 1 : # this is last word
                                    print('adding space')
                                    formatted_text += ' '
                                    if len(each_big_curr_word) + 1 == width - running_width: # fits perfectly after adding space 
                                        add_new_line = True
                                    else:
                                        running_width += len(each_big_curr_word) + 1
                                else: #more words to come 
                                    print('not perfect fit')
                                    running_width += len(each_big_curr_word)'''
                else:
                    #print('words insert',words[n:])
                    lines.insert(0,''.join(words[n:]))
                    break
...
def build_cell_block(cell_properties):
    ...
    build_cell_text(print_arr,cell_properties)
    '''for i in print_arr:
        print(''.join(i))
    #input()'''
    return print_arr
